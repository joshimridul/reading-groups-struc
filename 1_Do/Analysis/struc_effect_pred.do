...
loc full "meanpeerscore realised_level levelXind_hat classsize" 
		...
		forvalues s = 1/6 {
		    ...
		    if `s' == 6 {
				loc indep "meanpeerscore realised_level classsize levelXind_hat"
				loc exp "exp_meanpeerscore exp_level exp_classsize"
			} 
...
*gen upperXpred = upper*std_endline_hat
...
loc full "meanpeerscore realised_level levelXind_hat classsize" 
		...
		forvalues s = 1/6 {
			...
			reghdfe std_score_el `indep' `exp' std_endline_hat i.grade, a(strata) vce(clus ggroup)
...
file open let using "${outdir}/structural_testscores.tex", write replace	
	...
	*file write let "`S'  \\" _n
	...
	file write let "\item Notes: All specifications control for individual predicted endline test score, grade dummies, and randomization strata fixed effects. Each specification also controls for the expected value of the respective independent variable used in the specification viz. peer predicted score, level and class size. Standard errors are clustered at the academy-grade group level. ***, **, and * indicate significance at 1\%, 5\%, and 10\%." 
