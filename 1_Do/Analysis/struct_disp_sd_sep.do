...
loc full "meanpeerscore sdpeerscore dev_med_ehat classsize" 
		...
		forvalues s = 1/7 {
			...
			if `s'== 2 {
				loc indep "sdpeerscore"
				loc exp "exp_sdpeerscore"
			}

		    if `s' == 3 {
				loc indep "dev_med_ehat"
				loc exp "exp_devmed_ehat"
			} 
		    ...
		    if `s' == 5 {
				loc indep "meanpeerscore dev_med_ehat classsize"
				loc exp "exp_meanpeerscore exp_devmed_ehat exp_classsize"
			} 

			if `s' == 6 {
				loc indep "sdpeerscore dev_med_ehat classsize"
				loc exp "exp_sdpeerscore exp_devmed_ehat exp_classsize"
			}


			if `s' == 7 {
				loc indep "meanpeerscore sdpeerscore dev_med_ehat classsize"
				loc exp "exp_meanpeerscore exp_sdpeerscore exp_devmed_ehat exp_classsize"
			}

			*reghdfe std_comp_score_el `indep' `exp' std_endline_hat i.grade, a(constituency) vce(clus academycode)
			reghdfe std_comp_score_el `indep' `exp' std_comp_score_bl i.grade, a(constituency) vce(clus academycode)
...
file open let using "${outdir}/distance_median_sd_sep.tex", write replace	
	file write let "\begin{center} " _n
	file write let "\caption{\small Effect on test scores (by grade groups)} " _n
	...
	*file write let "`S'  \\" _n
	...
	file write let "\item Notes: All specifications control for baseline test score, academy cohort dummies and grade dummies. Each specification also controls for the expected value of the respective independent variable used in the specification viz. peer baseline score, level and class size. Standard errors are clustered at the academy level. In the final period control group, one standard deviation is roughly equivalent to an 18\% increase in test scores in Kenya and 23\% in Liberia. ***, **, and * indicate significance at 1\%, 5\%, and 10\%." 
