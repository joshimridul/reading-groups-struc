...
loc full "meanpeerscore sdpeerscore dev_med_ehat classsize" 
		...
		forvalues s = 1/7 {
			...
			*reghdfe std_comp_score_el `indep' `exp' std_endline_hat i.grade, a(constituency) vce(clus academycode)
			reghdfe dev_mean_c `indep' `exp' std_comp_score_bl i.grade, a(constituency) vce(clus academycode)
...
loc full "meanpeerscore sdpeerscore dev_med_ehat classsize" 
		...
		forvalues s = 1/7 {
			...
			reghdfe dev_mean_c `indep' `exp' std_score_bl i.grade , a(strata) vce(clus ggroup)
...
file open let using "${outdir}/eff_on_disp_c.tex", write replace	
	file write let "\begin{center} " _n
	file write let "\caption{\small Effect on absolute deviation in test scores from class mean} " _n
