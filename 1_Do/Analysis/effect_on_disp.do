...
forval g = 0/2 {
	...
	foreach x in `xlist' {
			...
			if "`x'" == "dev_mean_bl" {
				reghdfe `x' treat if gg`g' == 1, a(constituency) vce(clus academycode)
			}

			else {
				reghdfe `x' treat std_comp_score_bl if gg`g' == 1, a(constituency) vce(clus academycode)
			}
...
forval g = 1/4 {
	...
	foreach x in `xlist' {
			...
			if "`x'" == "dev_mean_bl" {
				reghdfe `x' treat if g`g' == 1, a(strata) vce(clus ggroup)
			}

			else {
				reghdfe `x' treat std_score_bl if g`g' == 1, a(strata) vce(clus ggroup)
			}
