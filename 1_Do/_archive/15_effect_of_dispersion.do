...
Title: 15_effect_of_dispersion.do 
Author: Mridul Joshi
Date: Mon Mar  7 17:59:10 2022

Description: Dispersion as a dependent variable
...
lab var ind_hatXpeer_hat "Peer pred. score \times Indivi pred. score"
...
** actual outcome 
bys academycode grade grade_stream: egen mean_el_c = mean(std_endline_hat)
bys academycode purple grade_stream: egen mean_el_t = mean(std_endline_hat)

gen dev_mean_t = abs(std_endline_hat -  mean_el_t)
gen dev_mean_c = abs(std_endline_hat -  mean_el_c)

gen exp_devmean = (P_t*dev_mean_t) + (P_c*dev_mean_c)
lab var exp_devmean "E[dispersion]"

gen mean_el = mean_el_c
replace mean_el = mean_el_t if treat == 1 

gen dev_mean_el = abs(std_endline_hat - mean_el)
lab var dev_mean_el "Dispersion"

* create other variables 
gen ind_hatXdisp = std_endline_hat*dev_mean_el
lab var ind_hatXdisp "Dispersion \times Ind. pred. score"


loc full "dev_mean_el ind_hatXdisp realised_level levelXind_hat classsize classizeXind_hat" 


		forvalues s = 1/8 {
		
			if `s'== 1 {
				loc indep "dev_mean_el"
				loc exp "exp_devmean"
			}
			if `s' == 2 {
				loc indep "dev_mean_el ind_hatXdisp"
				loc exp "exp_devmean"
			} 
		    if `s' == 3 {
		    ...
		    if `s' == 7 {
				loc indep "dev_mean_el realised_level classsize"
				loc exp "exp_devmean exp_level exp_classsize"
			} 
		    if `s' == 8 {
				loc indep "dev_mean_el realised_level classsize ind_hatXdisp levelXind_hat classizeXind_hat"
				loc exp "exp_devmean exp_level exp_classsize"
			} 
...
gen ind_hatXpeer_hat = std_endline_hat*meanpeerscore
lab var ind_hatXpeer_hat "Dispersion $\times$ Individual predicted score"
...
** actual outcome 
bys academycode grade stream: egen mean_el_c = mean(std_endline_hat)
bys academycode std_grp stream: egen mean_el_t = mean(std_endline_hat)
...
* create other variables 
gen ind_hatXdisp = std_endline_hat*dev_mean_el
lab var ind_hatXdisp "Dispersion $\times$ Individual predicted score"
...
file open let using "${outdir}/struc_testscores_disp.tex", write replace	
	file write let "\begin{center} " _n
	file write let "\caption{\small Effect of within-class dispersion on test scores} " _n
