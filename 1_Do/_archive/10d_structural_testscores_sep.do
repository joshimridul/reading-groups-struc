...
loc full "meanpeerscore peer_hatXind_hat realised_level levelXind_hat classsize classizeXind_hat" 
		...
		forvalues s = 1/8 {
		    ...
		    if `s' == 8 {
				loc indep "meanpeerscore realised_level classsize peer_hatXind_hat levelXind_hat classizeXind_hat"
				loc exp "exp_meanpeerscore exp_level exp_classsize"
			}	
...
file open let using "${outdir}/structural_testscores_sep.tex", write replace	
	file write let "\begin{center} " _n
	file write let "\caption{\small Effect on test scores (Liberia separated)} " _n
