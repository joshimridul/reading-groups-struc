...
bys academycode purple grade_stream: egen N_t1 = count(studyid) if inlist(data_avl,2,3)
bys academycode grade grade_stream: egen N_c1 = count(studyid)  if inlist(data_avl,2,3)
...
loc full "meanpeerscore ind_hatXpeer_hat realised_level levelXind_hat classsize classizeXind_hat" 
		...
		forvalues s = 1/8 {
			...
			if `s' == 2 {
				loc indep "meanpeerscore ind_hatXpeer_hat"
				loc exp "exp_meanpeerscore"
			} 
		    if `s' == 3 {
		    ...
		    if `s' == 4 {
				loc indep "realised_level levelXind_hat"
				loc exp "exp_level"
			} 
		    if `s' == 5 {
				loc indep "classsize"
		    ...
		    if `s' == 6 {
				loc indep "classsize classizeXind_hat"
				loc exp "exp_classsize"
			} 
		    if `s' == 7 {
		    ...
		    if `s' == 8 {
				loc indep "meanpeerscore realised_level classsize ind_hatXpeer_hat levelXind_hat classizeXind_hat"
				loc exp "exp_meanpeerscore exp_level exp_classsize"
			} 

			*reghdfe std_score_el `indep' `exp' std_endline_hat std_score_bl i.grade test_type, a(academy_cohort) vce(clus academycode)
			reghdfe std_score_el `indep' `exp' std_endline_hat i.grade test_type, a(academy_cohort) vce(clus academycode)
...
gen bl_dummy = mi(score_bl)

*bys academycode grade stream: egen mean_bl = mean(score_bl)

*replace score_bl = mean_bl if mi(score_bl)
...
reg score_el score_bl score_ml score_ml_gr* score_bl_gr? grade_dum? if treat == 0
*predict endline_hat, xb
...
loc full "meanpeerscore ind_hatXpeer_hat realised_level levelXind_hat classsize classizeXind_hat" 
		...
		forvalues s = 1/8 {
			...
			*reghdfe std_score_el `indep' `exp' std_score_bl std_endline_hat i.grade, a(academy_cohort county) vce(clus ggroup)
			reghdfe std_score_el `indep' `exp' std_endline_hat i.grade, a(academy_cohort) vce(clus ggroup)
...
*gen score_bl1 = score_breplace score_bl = mean_bl if mi(score_bl)
...
*bys academycode grade stream: egen mean_bl = mean(endline_hat)
*replace endline_hat = mean_bl if mi(endline_hat)
...
loc full "meanpeerscore ind_hatXpeer_hat realised_level levelXind_hat classsize classizeXind_hat" 
		...
		forvalues s = 1/8 {
			...
			*reghdfe std_score_el `indep' `exp' std_score_bl std_endline_hat i.grade, a(academy_cohort county) vce(clus ggroup)
			reghdfe std_score_el `indep' `exp' std_endline_hat i.grade, a(academy_cohort) vce(clus ggroup)  // worked
			*reghdfe std_score_el `indep' `exp' std_endline_hat, a(academy_cohort) vce(clus ggroup)  // worked

			*reghdfe std_score_el `indep' `exp' std_endline_hat i.grade, a(academy_cohort county) vce(clus ggroup)
...
file open let using "${outdir}/structural_testscores_rev.tex", write replace	
	file write let "\begin{center} " _n
	file write let "\caption{\small Effect on test scores (by grade group, baseline classsize)} " _n
	...
	*file write let "`S'  \\" _n
	...
	file write let "\item Notes: All specifications control for baseline test score, individual predicted test score, academy cohort dummies and grade dummies. Each specification also controls for the expected value of the respective independent variable used in the specification viz. peer predicted score, level and class size. Standard errors are clustered at the academy-grade group level. In the final period control group, one standard deviation is roughly equivalent to an 18\% increase in test scores in Kenya and 23\% in Liberia. ***, **, and * indicate significance at 1\%, 5\%, and 10\%." 
