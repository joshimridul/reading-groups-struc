...
Description: Pooled structural estimates 
...
use "${datadir}/Liberia/6_L_AG_full_stacked.dta", clear 
...
duplicates drop academycode studyid, force  // change to a collapse statement 
...
reg score_mlel score_bl score_bl_gr? grade_dum? if treat == 0
predict endline_hat, xb
...
gen test_type = test_name ==1 


** actual outcome 
bys academycode grade stream: egen mean_el_c = mean(std_score_mlel)
bys academycode std_grp stream: egen mean_el_t = mean(std_score_mlel)

gen mean_el = mean_el_c
replace mean_el = mean_el_t if treat == 1 

gen dev_mean_el = abs(std_score_mlel - mean_el)
...
loc full "meanpeerscore ind_hatXpeer_hat realised_level levelXind_hat classsize classizeXind_hat" 
		...
		forvalues s = 1/8 {
			...
			*reghdfe dev_mean_el `indep' `exp' std_endline_hat std_score_bl i.grade test_type, a(academy_cohort) vce(clus ggroup)
			reghdfe dev_mean_el `indep' `exp' std_endline_hat i.grade test_type, a(academy_cohort) vce(clus ggroup)
...
file open let using "${outdir}/structural_dispersion_pooled.tex", write replace	
	file write let "\begin{center} " _n
	file write let "\caption{\small Effect on within-class dispersion (Pooling all available post-intervention test scores)} " _n
