...
loc full "meanpeerscore ind_hatXpeer_hat realised_level levelXind_hat classsize classizeXind_hat" 
		...
		forvalues s = 1/8 {
			...
			*reghdfe std_score_mlel `indep' `exp' std_endline_hat std_score_bl i.grade test_type, a(academy_cohort) vce(clus ggroup)
			reghdfe std_score_mlel `indep' `exp' std_endline_hat  i.grade test_type, a(academy_cohort) vce(clus ggroup)
...
file open let using "${outdir}/structural_pooled.tex", write replace	
	file write let "\begin{center} " _n
	file write let "\caption{\small Effect on test scores (Pooling all available post-intervention test scores)} " _n
