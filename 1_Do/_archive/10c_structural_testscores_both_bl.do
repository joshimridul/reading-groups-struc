...
loc full "meanpeerscore_bl peer_hat_blXbl dev_mean_bl  dispXbl realised_level levelXbl classsize classizeXbl" 
		...
		forvalues s = 1/10 {
			...
			reghdfe std_score_el `indep' `exp' std_score_bl i.grade, a(academy_cohort) vce(clus academycode)
...
file open let using "${outdir}/structural_testscores_both_bl.tex", write replace	
	file write let "\begin{center} " _n
	file write let "\caption{\small Effect on test scores (peer baseline score and dispersion)} " _n
	...
	*file write let "`S'  \\" _n
	...
	file write let "\item Notes: All specifications control for baseline test score, academy cohort dummies and grade dummies. Each specification also controls for the expected value of the respective independent variable used in the specification viz. peer baseline score, dispersion, level and class size. Standard errors are clustered at the academy level. In the final period control group, one standard deviation is roughly equivalent to an 18\% increase in test scores in Kenya and 23\% in Liberia. ***, **, and * indicate significance at 1\%, 5\%, and 10\%." 
