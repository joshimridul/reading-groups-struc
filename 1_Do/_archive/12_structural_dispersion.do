...
Title: 12_structural_dispersion.do 
Author: Mridul Joshi
Date: Thu Feb  3 09:20:35 2022

Description: Test score dispersion structural estimates
...
bys academycode purple grade_stream: egen N_t = count(studyid) 


bys academycode grade grade_stream: egen N_c = count(studyid) 
...
** actual outcome 
bys academycode grade grade_stream: egen mean_el_c = mean(std_comp_score_el)
bys academycode purple grade_stream: egen mean_el_t = mean(std_comp_score_el)
...
loc full "meanpeerscore ind_hatXpeer_hat realised_level levelXind_hat classsize classizeXind_hat" 
		...
		forvalues s = 1/8 {
			...
			*reghdfe dev_mean_el `indep' `exp' std_comp_score_bl std_endline_hat i.grade, a(academy_cohort) vce(clus academycode)
			reghdfe dev_mean_el `indep' `exp'  std_endline_hat i.grade, a(academy_cohort) vce(clus academycode)
...
bys academycode std_grp stream: egen N_t = count(studyid)
bys academycode grade stream: egen N_c = count(studyid)
...
file open let using "${outdir}/structural_dispersion.tex", write replace	
	file write let "\begin{center} " _n
	file write let "\caption{\small Effect on within-class dispersion of test scores (final period test scores)} " _n
	...
	*file write let "`S'  \\" _n
	...
	file write let "\item Notes: The outcome is the absolute deviation of each student's standardised test score from the classroom mean. All specifications control for baseline test score, individual predicted test score, academy cohort dummies and grade dummies. Each specification also controls for the expected value of the respective independent variable used in the specification viz. peer predicted score, level and class size. Standard errors are clustered at the academy-grade group level. In the final period control group, one standard deviation is roughly equivalent to an 18\% increase in test scores in Kenya and 23\% in Liberia. ***, **, and * indicate significance at 1\%, 5\%, and 10\%." 
