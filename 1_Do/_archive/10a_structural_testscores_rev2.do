...
loc full "meanpeerscore ind_hatXpeer_hat realised_level levelXind_hat classsize classizeXind_hat" 
		...
		forvalues s = 1/8 {
			...
			reghdfe std_score_el `indep' `exp' std_score_bl i.grade bl_dummy, a(academy_cohort county) vce(clus ggroup)
			*reghdfe std_score_el `indep' `exp' std_endline_hat i.grade, a(academy_cohort) vce(clus ggroup)
...
file open let using "${outdir}/structural_testscores_rev2.tex", write replace	
	file write let "\begin{center} " _n
	file write let "\caption{\small Effect on test scores (by grade group, all observation classsize)} " _n
