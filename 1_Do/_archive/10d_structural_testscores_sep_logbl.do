...
loc full "meanpeerscore_bl peer_hat_blXbl realised_level levelXbl logclasssize logclassizeXbl" 
		...
		forvalues s = 1/8 {
		    ...
		    if `s' == 7 {
				loc indep "meanpeerscore_bl realised_level logclasssize"
				loc exp "exp_meanpeerscore_bl exp_level exp_logclasssize"
			} 
		    if `s' == 8 {
				loc indep "meanpeerscore_bl realised_level logclasssize peer_hatXbl levelXbl logclassizeXbl"
				loc exp "exp_meanpeerscore_bl exp_level exp_logclasssize"
			} 
...
file open let using "${outdir}/structural_testscores_sep_logbl.tex", write replace	
	file write let "\begin{center} " _n
	file write let "\caption{\small Effect on test scores (by grade groups; log classize)} " _n
	...
	*file write let "`S'  \\" _n
	...
	file write let "\item Notes: All specifications control for baseline test score, academy cohort dummies and grade dummies. Each specification also controls for the expected value of the respective independent variable used in the specification viz. peer baseline score, level and log class size. Standard errors are clustered at the academy level. In the final period control group, one standard deviation is roughly equivalent to an 18\% increase in test scores in Kenya and 23\% in Liberia. ***, **, and * indicate significance at 1\%, 5\%, and 10\%." 
