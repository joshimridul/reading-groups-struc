...
loc full "dev_mean_el dispXind_hat realised_level levelXind_hat classsize classizeXind_hat" 
		...
		forvalues s = 1/8 {
			...
			if `s' == 2 {
				loc indep "dev_mean_el dispXind_hat"
				loc exp "exp_devmean"
			} 
		    if `s' == 3 {
		    ...
		    if `s' == 8 {
				loc indep "dev_mean_el realised_level classsize dispXind_hat levelXind_hat classizeXind_hat"
				loc exp "exp_devmean exp_level exp_classsize"
			} 
...
file open let using "${outdir}/structural_testscores_disp_sep.tex", write replace	
	file write let "\begin{center} " _n
	file write let "\caption{\small Effect on test scores (Dispersion; Liberia separated)} " _n
