...
loc full "dev_mean_bl dispXbl realised_level levelXbl classsize classizeXbl" 
		...
		forvalues s = 1/8 {
		    ...
		    if `s' == 7 {
				loc indep "dev_mean_bl realised_level classsize"
				loc exp "exp_devmean_bl exp_level exp_classsize"
			} 
		    if `s' == 8 {
				loc indep "dev_mean_bl realised_level classsize dispXbl levelXbl classizeXbl"
				loc exp "exp_devmean_bl exp_level exp_classsize"
			} 
...
file open let using "${outdir}/structural_testscores_disp_bl.tex", write replace	
	file write let "\begin{center} " _n
	file write let "\caption{\small Effect on test scores (with dispersion)} " _n
	...
	*file write let "`S'  \\" _n
	...
	file write let "\item Notes: All specifications control for baseline test score, academy cohort dummies and grade dummies. Each specification also controls for the expected value of the respective independent variable used in the specification viz. dispersion, level and class size. Standard errors are clustered at the academy level. In the final period control group, one standard deviation is roughly equivalent to an 18\% increase in test scores in Kenya and 23\% in Liberia. ***, **, and * indicate significance at 1\%, 5\%, and 10\%." 
