...
gen has_anyscore = inlist(pupil_observed,3)
...
forval g = 0/2 {
			...
			summ has_anyscore if treat == 0 & g`g' == 1
			...
			loc star = ""
			cap areg has_anyscore treat i.grade std_comp_score_bl if g`g' == 1, a(constituency) vce(clus academycode) // bl and el
...
gen has_el = (pupil_observed == 6 | pupil_observed == 7)


forval g = 0/2 {

			sum has_el if treat == 0 & g`g' == 1

			local mw`g'l0 : display string(r(mean),"%9.3f")
			local Nmw`g'l0 : display string(r(N),"%9.0fc")

			areg has_el treat i.grade std_score_bl if g`g'==1, vce(clus ggroup) a(strata) 
