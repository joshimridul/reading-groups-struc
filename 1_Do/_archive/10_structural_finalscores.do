...
loc full "meanpeerscore ind_hatXpeer_hat realised_level levelXind_hat classsize classizeXind_hat" 
		...
		forvalues s = 1/8 {
			...
			*reghdfe std_comp_score_el `indep' `exp' std_comp_score_bl std_endline_hat i.grade, a(academy_cohort) vce(clus academycode)
			reghdfe std_comp_score_el `indep' `exp' std_endline_hat i.grade, a(academy_cohort) vce(clus academycode)
...
loc full "meanpeerscore ind_hatXpeer_hat realised_level levelXind_hat classsize classizeXind_hat" 
		...
		forvalues s = 1/8 {
			...
			*reghdfe std_score_el `indep' `exp' std_score_bl std_endline_hat i.grade, a(academy_cohort) vce(clus ggroup)
...
file open let using "${outdir}/structural_finalscores.tex", write replace	
	file write let "\begin{center} " _n
	file write let "\caption{\small Effect on test scores (final period test scores)} " _n
