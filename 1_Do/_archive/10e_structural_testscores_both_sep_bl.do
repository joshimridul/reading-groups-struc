...
loc full "meanpeerscore_bl peer_hat_blXbl dev_mean_bl  dispXbl realised_level levelXbl classsize classizeXbl" 
		...
		forvalues s = 1/10 {
			...
			if `s'== 2 {
				loc indep "meanpeerscore_bl peer_hat_blXbl"
				loc exp "exp_meanpeerscore_bl"
			}
		    ...
		    if `s' == 8 {
				loc indep "classsize classizeXbl"
				loc exp "exp_classsize"
			} 

			if `s' == 9 {
				loc indep "meanpeerscore_bl dev_mean_bl realised_level classsize"
				loc exp "exp_meanpeerscore_bl exp_devmean_bl exp_level exp_classsize"
			} 

		    if `s' == 10 {
				loc indep "meanpeerscore_bl dev_mean_bl realised_level classsize peer_hat_blXbl dispXbl levelXbl classizeXbl"
				loc exp "exp_meanpeerscore_bl exp_devmean_bl exp_level exp_classsize"
			} 
...
file open let using "${outdir}/structural_testscores_both_sep_bl.tex", write replace	
	file write let "\begin{center} " _n
	file write let "\caption{\small Effect on test scores (by grade groups; peer baseline score and dispersion)} " _n
	file write let "\label{tab:descriptives} " _n
	file write let "\begin{`size'}" _n
	file write let "\begin{threeparttable}" _n
	file write let "\begin{tabular} {@{} l l c c c c c c c c c c c c @{}} \toprule \toprule" _n
