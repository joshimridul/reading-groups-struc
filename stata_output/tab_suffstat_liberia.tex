\begin{table}[H]
\centering
\caption{Peer/Rank Diagnostics and Classroom Shifts: Liberia}
...
\footnotesize
\item \textit{Notes:} Panel A reports the approximate Borusyak--Hull control function peer/rank coefficient using decile $\times$ treatment $\times$ grade FE\@. Standard errors clustered at the school grade group level. Panel B reports treatment control differences in classroom characteristics. The near-zero peer/rank coefficient and small classroom composition shift provide little evidence that the measured peer/rank channel explains the negative Liberia ITT pattern.
\end{tablenotes}
\end{threeparttable}
\end{table}
